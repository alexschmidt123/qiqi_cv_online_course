...
def render(video: Path, library_path: Path, gaze_path: Path, output_dir: Path, count: int) -> None:
    ...
    gaze_rows = _sample_rows(_load_gaze(gaze_path), count)
    ...
    for index, (timestamp, x_bl, y_bl) in enumerate(gaze_rows, 1):
        ...
        if not ok or frame is None:
            continue
        h, w = frame.shape[:2]
        x_tl, y_tl = x_bl, (h - 1) - y_bl
        nx, ny = x_tl / width, y_tl / height
        state = _state_at(states, timestamp)
        element = _element_at(state, nx, ny) if state else None
        ...
        _draw_text_panel(canvas, [f"Time: {timestamp:.3f}s | Visible state: {state_name}", *_decision(state, element, nx, ny)])
        cv2.imwrite(str(output_dir / f"validation_{index:03d}_{timestamp:.3f}s.png"), canvas)
    cap.release()
